...
float cnoise(float4 P)
{
  ...
  gy00 = frac(gy00) - 0.5;
  gz00 = frac(gz00) - 0.5;
  float4 gw00 = MFLOAT4(0.75) - abs(gx00) - abs(gy00) - abs(gz00);
  float4 sw00 = step(gw00, MFLOAT4(0.0));
  ...
  gy01 = frac(gy01) - 0.5;
  gz01 = frac(gz01) - 0.5;
  float4 gw01 = MFLOAT4(0.75) - abs(gx01) - abs(gy01) - abs(gz01);
  float4 sw01 = step(gw01, MFLOAT4(0.0));
  ...
  gy10 = frac(gy10) - 0.5;
  gz10 = frac(gz10) - 0.5;
  float4 gw10 = MFLOAT4(0.75) - abs(gx10) - abs(gy10) - abs(gz10);
  float4 sw10 = step(gw10, MFLOAT4(0.0));
  ...
  gy11 = frac(gy11) - 0.5;
  gz11 = frac(gz11) - 0.5;
  float4 gw11 = MFLOAT4(0.75) - abs(gx11) - abs(gy11) - abs(gz11);
  float4 sw11 = step(gw11, MFLOAT4(0.0));
...
void SG_classic4D_float( float4 i, out float o )
{
    o = cnoise( i );
}

void SG_classic4D_periodic_float( float4 i, float4 r, out float o )
{
    o = pnoise( i, r );
}